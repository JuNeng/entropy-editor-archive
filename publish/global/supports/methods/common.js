...
var commonFun = {
  ...
  channel_city: {
    ...
    author_list: function author_list(obj) {
      ...
      return (authorData = authorData.substr(
        authorData,
        authorData.length - 1
      ));
    }
  ...
  Validation: {
    isTelephone: function isTelephone(str) {
      if (
        str.match(/^(([0\+]\d{2,3}-)?(0\d{2,3})-)(\d{7,8})(-(\d{3,}))?$/) ==
        null
      ) {
        return false;
      } else {
    ...
    isDate: function isDate() {
      ...
      if (value == null) {
        ...
        return (
          date.getFullYear() == value[1] &&
          date.getMonth() + 1 == value[3] &&
          date.getDate() == value[4]
        );
      }
    },
  ...
  Url: {
    ...
    getParamByReverseUrlIndex: function getParamByReverseUrlIndex(i) {
      ...
      var url = href.substring(
        href.lastIndexOf("http://") + 7,
        href.lastIndexOf("/") || href.lastIndexOf("/?")
      );
      url = url.split("/");
      url.reverse();
  ...
  Array: {
    ...
    mergeDedupe: function mergeDedupe(arr) {
      ...
      return [].concat(
        _toConsumableArray(
          new Set((_ref = []).concat.apply(_ref, _toConsumableArray(arr)))
        )
      );
    },
