...
var UploadImage = (function(_Component) {
  ...
  function UploadImage(props) {
    ...
    var _this2 = _possibleConstructorReturn(
      this,
      (UploadImage.__proto__ || Object.getPrototypeOf(UploadImage)).call(
        this,
        props
      )
    );
    ...
    _this2.state = {
      ...
      qiniu: {
        token:
          _this2.props.uploadConfig &&
          Object.keys(_this2.props.uploadConfig).length
            ? _publicDatas.PRO_QINIU.checkQiniu.returnToken(
                _this2.props.uploadConfig
              )
            : null
      },
  ...
  _createClass(UploadImage, [
    ...
    },
    {
      ...
      value: function getInputVideo(e) {
        ...
        var value = e.target.value;
        if (
          _publicDatas.PRO_COMMON.Validation.isWebFileURL(
            value,
            this.props.fileType
          )
        ) {
          ...
          this.forceUpdate();
          setTimeout(function() {
            _this3.state.files.push({
              url: value,
              name: value,
              status: "done",
              uid: "uid_" + _publicDatas.PRO_COMMON.String.RndNum(20)
            });
    ...
    },
    {
      ...
      value: function beforeUpload(file) {
        var isFormat = _publicDatas.PRO_COMMON.Array.inArray(
          _publicDatas.PRO_QINIU.supportMime[this.props.fileType],
          file.type
        );
        if (!isFormat) {
          _message2.default.error(
            this.props.lang.supportMimeMsg +
              " File Mimetype: " +
              _publicDatas.PRO_QINIU.supportMime[this.props.fileType].join(
                ", "
              ),
            10
          );
        ...
        if (!this.state.qiniu.token && !uploadConfig) {
          var token = _publicDatas.PRO_QINIU.checkQiniu.returnToken(
    ...
    },
    {
      ...
      value: function onChange(info) {
        ...
        if (this.props.fileType == "image") {
          url =
            _publicDatas.PRO_URL.QINIU_DOMAIN_IMG_URL ||
          ...
          url =
            _publicDatas.PRO_URL.QINIU_DOMAIN_VIDEO_URL ||
        ...
        fileList = fileList.filter(function(file) {
          var hasNoExistCurrFileInUploadedList = !~(0, _findIndex2.default)(
            _this.state.files,
            function(item) {
      ...
      value: function render() {
        ...
        uploadProps =
          uploadProps && Object.keys(uploadProps).length > 0
            ...
            : {
                action:
                  _publicDatas.PRO_URL.QINIU_URL ||
                  this.props.uploadConfig.QINIU_URL,
                ...
                data: function data(file) {
                  var token = that.state.qiniu.token,
                    key = "";
                  if (!token) {
                    token = _publicDatas.PRO_QINIU.checkQiniu.returnToken(
                      _this6.props.uploadConfig
                    );
                  }
                  key =
                    _publicDatas.PRO_COMMON.String.RndNum(20) +
                    "." +
                    _publicDatas.PRO_COMMON.String.GetFileExtensionName(
                      file.name
                    )[0];
                  if (_this6.props.uploadConfig.QINIU_KEY_PREFIX) {
                    key =
                      _this6.props.uploadConfig.QINIU_KEY_PREFIX + "/" + key;
                  }
