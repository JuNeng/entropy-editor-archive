  ...
  _createClass(BlockGenerator, [
        ...
        switch (tagName) {
          case "li": {
            ...
            return parent.tagName === "ol"
              ? _main.BLOCK_TYPE.ORDERED_LIST_ITEM
              : _main.BLOCK_TYPE.UNORDERED_LIST_ITEM;
          }
          case "blockquote": {
    ...
    },
    {
      ...
      value: function processInlineElement(element) {
        ...
        style = addStyleFromTagName(
          style,
          tagName,
          this.options.elementStyles,
          element
        );
  ...
  switch (tagName) {
    ...
    case "span": {
      ...
      if (savedColor.lastIndexOf("rgb") > -1) {
        savedColor = savedColor.substring(
          savedColor.lastIndexOf("(") + 1,
          savedColor.length - 1
        );
        savedColor = savedColor.split(",");
      }
      var savedHex = rgbToHex(
        parseInt(savedColor[0]),
        parseInt(savedColor[1]),
        parseInt(savedColor[2])
      );
      var savedKey = "";
      Object.keys(_colorConfig.colorStyleMap).map(function(key) {
        if (
          _colorConfig.colorStyleMap[key].color.toLowerCase() ==
          savedHex.toLowerCase()
        ) {
          savedKey = key;
        }
      });
