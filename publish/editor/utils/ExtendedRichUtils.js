...
var _getCurrentlySelectedBlock2 = _interopRequireDefault(
  _getCurrentlySelectedBlock
);
...
var ExtendedRichUtils = Object.assign({}, _draftJs.RichUtils, {
  toggleAlignment: function toggleAlignment(editorState, alignment) {
    var _getCurrentlySelected = (0, _getCurrentlySelectedBlock2.default)(
        editorState
      ),
    ...
    var alignmentToSet =
      !!blockData && keyName === alignment ? undefined : alignment;

    var alignBlockData = new Map();
    alignBlockData.set(ALIGNMENT_DATA_KEY, alignmentToSet);

    var newBlockData = _draftJs.Modifier.setBlockData(
      content,
      target,
      alignBlockData
    );

    return _draftJs.EditorState.push(
      editorState,
      newBlockData,
      "change-block-type"
    );
  ...
  splitBlock: function splitBlock(editorState) {
    var contentState = _draftJs.Modifier.splitBlock(
      editorState.getCurrentContent(),
      editorState.getSelection()
    );
    var splitState = _draftJs.EditorState.push(
      editorState,
      contentState,
      "split-block"
    );

    var _getCurrentlySelected2 = (0, _getCurrentlySelectedBlock2.default)(
        editorState
      ),
