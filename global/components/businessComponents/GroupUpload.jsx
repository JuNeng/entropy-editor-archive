...
class GroupUpload extends Component {
  ...
  getPfop() {
    ...
    let newPicturesObj = this.state.selectedPictureList.map(item => {
      ...
      if (!!item && !~item.lastIndexOf("reset")) {
        ...
        let extensionNameItem = originKey.match(/\.[^\.]*/g),
          extensionName = "";
        ...
        return {
          ...
          originPic:
            ...
            "/image/" +
            find(
              PRO_BASE.Config.watermarkImage,
              item => item.type == this.state.selectedWaterMarkType
            ).valuebase64 +
            "/dx/50/dy/50",
          newName:
            (~originKey.lastIndexOf("QN1D")
              ...
              : originKey +
                ...
                this.state.selectedWaterMarkPositon) + (extensionName || ""),
          key: originKey,
    ...
    remove(newPicturesObj, item => {
      return !item.originPic;
    });
    let removedPic = remove(newPicturesObj, item => {
  ...
  getPictures(listPicture) {
    ...
    this.state.pictureList = compact(
  ...
  componentWillReceiveProps(prevProps, nextProps) {
    ...
    if (!!nextProps && nextProps.hasOwnProperty("imageList")) {
      this.setState({
        ...
        selectedPictureList: cloneDeep(nextProps.imageList)
      });
    } else {
      ...
      <span>
        {(() => {
          if (!this.props.children) {
            return <Button onClick={this.openModal}>{lang.btnAddBatch}</Button>;
          } else {
            const childrenWithProps = React.Children.map(
              this.props.children,
              child => React.cloneElement(child, { onClick: this.openModal })
            );
        ...
        <Modal
          title={
            <span>
              {" "}
              <span>{lang.batchAddModalTitle}</span> &nbsp; &nbsp; &nbsp; &nbsp;
              <Checkbox
