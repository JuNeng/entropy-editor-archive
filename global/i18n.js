module.exports = {
  lang: {
    en: {
      ...
      preview: "prev",
      selectedText:
        "Please make sure you have selected text before add a new link!",
      selectedLink:
        "Please make sure you have selected a link before remove it!",
      successToDraftBox: "Context have been saved to Draft-Box",
      successPasteCleanText:
        "Pasted no-style text success, it would not be display if the image url is 'Anti-Leech'.",
      errorUploadingFile: "!!!!!!!!!!!!ERROR UPLOADING!!!!!!!!!!!!",
      directToURL: "Please input a valid url",
      directToURLTip:
        'Please enter the regular URL(with "http://" or "https://" preffix.)',
      ...
      inPreviewProgress:
        "In processing and generate preview, 1 minute please...",
      ...
      draftTipMsg:
        "Draft Box--you can use Ctrl+S or Cmd+S save content to draft box, click to open.",
      draftModalTitle: "Draft item list",
      confirmUseDraft:
        "Current editor context will be replaced by your selected draft item, continue?",
      draftCautionMsg:
        "Content auto save to draft box every minute, you could save it manually by Ctrl+S or Cmd+S, it would not saving while content length less than 20 characters.",
      ...
      width: "width",
      zoomTipMsg:
        "Zoomout or zoomin to $targetSize$ according image $accordingSize$ ",
    ...
    "zh-CN": {
      ...
      successPasteCleanText:
        "已经清空样式并成功粘贴，可能部分图片因原网站防盗链功能暂未显示。",
      errorUploadingFile:
        "！！！！！！！！！！上传文件错误！！！！！！！！！！",
      ...
      deleteDraftItem: "删除",
      draftTipMsg:
        "保险库——编辑器中按Ctrl+S或Cmd+S存入保险库，点击打开保险库以继续",
      draftModalTitle: "编辑器保险库条目列表：",
      confirmUseDraft:
        "确定后编辑器内容将会被最后一次保存的内容替换，若有更改，替换后将不可恢复，是否继续？",
      draftCautionMsg:
      ...
      limitCountTip:
        "只能保留最后上传的 $limit$ 个文件，其他超出的已经被顶掉。",
      btnUpload: "点击上传",
    ...
    zh: {
      ...
      successPasteCleanText:
        "已經清空樣式並成功粘貼，可能部分圖片因原網站防盜鏈功能暫未顯示。",
      errorUploadingFile:
        "！！！！！！！！！！上傳文件錯誤！！！！！！！！！！",
      directToURL: "請輸出你要跳轉的連結",
      directToURLTip:
        "請輸入符合規範的網址連結（以“http://”或“https://”為前導）",
      ...
      deleteDraftItem: "删除",
      draftTipMsg:
        "保險庫——編輯器中按Ctrl+S或Cmd+S存入保險庫，點擊打開保險庫以繼續",
      draftModalTitle: "編輯器保險庫條目清單：",
      confirmUseDraft:
        "確定後編輯器內容將會被最後一次保存的內容替換，若有更改，替換後將不可恢復，是否繼續？",
      draftCautionMsg:
        "編輯器內容默認一分鐘保存一次，可以手動Ctrl+S或Cmd+S保存，內容小於20字時將不會保存在保險庫中；清單中不用數據請及時删除維護，否則會影響保險庫保存容量！",
      autoSave: "自動保存庫",
    ...
    ru: {
      ...
      successPasteCleanText:
        "Вставка теста без стиля выполнена, он не будет отображаться если URL картинки является 'Anti-Leech'.",
      errorUploadingFile: "!!!!!!!!!!!!ОШИБКА ЗАГРУЗКИ!!!!!!!!!!!!",
      directToURL: "Пожалуйста введите URL",
      directToURLTip:
        'Пожалуйста введите URL(с префиксом "http://" или "https://".)',
      ...
      confirmToRemove:
        "Вы действительно хотите удалить стили у выделенного текста?",
      doRemove: "Да",
      doNotRemove: "Нет",
      ...
      draftTipMsg:
        "Draft-Box - вы можете нажать Ctrl+S или Cmd+S для сохранения содержимого в хранилище черновиков.",
      draftModalTitle: "Список черновиков",
      confirmUseDraft:
        "Текущее содержимое редактора будет заменено содержимым выбранного черновика, продолжить?",
      draftCautionMsg:
        "Содержимое автоматически сохраняется в Draft-Box каждую минуту, вы можете выполнить сохранение вручную нажатием Ctrl+S or Cmd+S. Если длина содержимого менее 20 символов, автосохранение выполняться не будет.",
      ...
      manuallyUploadTip:
        "Вы можете вручную выбрать изображение, нажмите 'Enter' для подтверждения.",
      limitAndTypeTip:
        "Максимальное число загружаемых файлов: $limit$.($type$)",

      auto: "[AUTO]",
      height: "Высота",
      width: "Ширина",
      zoomTipMsg:
        "Уменьшение или увеличение до $targetSize$ в соответствии с $accordingSize$ ",
    ...
    ja: {
      ...
      successPasteCleanText:
        "スタイルのないテキストの貼り付けに成功しました, 画像のURLが不正な場合は表示されません",
      ...
      confirmUseDraft:
        "現在の入力内容が一時保存データで上書きされます よろしいですか?",
      draftCautionMsg:
        "入力内容は毎分自動で一時保存されます, Ctrl+S or Cmd+S で手動保存できます, コンテンツの長さが20文字未満の間は保存されません。",
      ...
      width: "幅",
      zoomTipMsg:
        "$accordingSize$ に応じて $targetSize$ に自動で拡大縮小します",
