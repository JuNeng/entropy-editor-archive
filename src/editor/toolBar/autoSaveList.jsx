...
import { Modal, Button, Popconfirm, message, Table, Icon } from "antd";
  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        ...
        key: "operation",
        render: (text, record) => (
          <a onClick={() => this.doDelete(record.keyName)}>
            {this.props.lang.deleteDraftItem}
          </a>
        )
      }
    ];
    ...
    return (
      <div className="RichEditor-controls">
        <span>
          <span
            className={className}
            onClick={that.onAutoSaveToggle}
            title={this.props.lang.draftTipMsg}
          >
            <Icon type="editor_safty" />
          </span>
        </span>
        <Modal
          ...
          footer={[
            ...
            <Popconfirm
              placement="right"
              title={this.props.lang.confirmUseDraft}
              onConfirm={that.sendSavedItemToEditor}
            >
              &nbsp;&nbsp;&nbsp;&nbsp;<Button
                key="submit"
                type="primary"
                size="large"
                disabled={!that.state.selectedRowKeys.length}
              >
                {" "}
